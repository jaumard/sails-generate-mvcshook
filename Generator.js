 ...
 * `sails generate mvcshook`
 *
 * @description Generates a mvcshook
...
module.exports = {
  ...
  before: function (scope, cb) {
    ...
    //
    // e.g. if someone runs:
    // $ sails generate mvcshook user find create update
    // then `scope.args` would be `['user', 'find', 'create', 'update']`
    if (!scope.args[0]) {
      return cb(new Error('Please provide a name for this mvcshook.'));
    }
...
function INVALID_SCOPE_VARIABLE(varname, details, message) {
  var DEFAULT_MESSAGE =
    'Issue encountered in generator "mvcshook":\n' +
    'Missing required scope variable: `%s`"\n' +
    'If you are the author of `sails-generate-mvcshook`, please resolve this ' +
